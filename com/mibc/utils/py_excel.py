...
@logger('保存启动时间测试结果')
def saveStartAppResult(cishu,start):
	try:
...
@logger('保存cpu，流量，内存')
def saveCpuResult(cishu,start_cpu,recv_list,send_list,total_list,Pass_list):
	try:
		workbook=xlsxwriter.Workbook('cpu_flow_mem_report.xlsx')
		worksheet=workbook.add_worksheet('cpu')
		worksheet_flow=workbook.add_worksheet('flow')
		worksheet_men=workbook.add_worksheet('men')
		bold=workbook.add_format({'bold':1})
		headings=['次数','cpu占用率']
		headings_flow=['次数','上传流量','下载流量','总计']
		headings_men=['次数','Pass占百分比']
		data_cpu=[cishu,start_cpu]
		data_flow=[cishu,recv_list,send_list,total_list]
		data_men=[cishu,Pass_list]
		worksheet_flow.write_row('A1',headings_flow,bold)
		worksheet_flow.write_column('A2',data_flow[0])
		worksheet_flow.write_column('B2',data_flow[2])
		worksheet_flow.write_column('C2',data_flow[1])
		worksheet_flow.write_column('D2',data_flow[3])
		...
		chart2.add_series({
			'name':'=flow!$B$1',
			'categories': '=flow!$A$2:$A$%s'%(len(cishu)+1),
			'values': '=flow!$B$2:$B$%s'%(len(cishu)+1),
		...
		chart2.add_series({
			'name':'=flow!$C$1',
			'categories':'=flow!$A$2:$A$%s'%(len(cishu)),
			 'values':'=flow!$C$2:$C$%s'%(len(cishu)),
			})
		chart2.add_series({
			'name':'=flow!$D$1',
			'categories':'=flow!$A$2:$A$%s'%(len(cishu)),
			 'values':'=flow!$D$2:$D$%s'%(len(cishu)),
			})
		...
		worksheet_flow.insert_chart('F2',chart2,{'x_offset': 60, 'y_offset': 60})
